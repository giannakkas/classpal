...
export default function ReviewPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      {/* Top bar */}
      <header className="bg-white border-b px-3 py-2 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <button onClick={() => router.push('/dashboard')} className="text-gray-500 hover:text-gray-700">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <h1 className="text-sm font-semibold text-gray-900 hidden sm:block">Review Paper</h1>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={undo} disabled={undoStack.length === 0} className="p-1.5 rounded hover:bg-gray-100 disabled:opacity-30 hidden sm:block">
            <Undo2 className="w-4 h-4" />
          </button>
          <button onClick={redo} disabled={redoStack.length === 0} className="p-1.5 rounded hover:bg-gray-100 disabled:opacity-30 hidden sm:block">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex items-center gap-1 px-2.5 py-1.5 text-xs sm:text-sm border rounded-lg hover:bg-gray-50"
          >
          ...
          <button
            ...
            disabled={finalizing}
            className="flex items-center gap-1 px-2.5 py-1.5 text-xs sm:text-sm bg-brand-600 text-white rounded-lg hover:bg-brand-700 disabled:opacity-50"
          >
            {finalizing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />}
            <span className="hidden sm:inline">{finalizing ? 'Generating...' : 'Finalize'}</span>
          </button>
          ...
          {paper.corrected_pdf_url && (
            <a
              ...
              target="_blank"
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs sm:text-sm bg-green-600 text-white rounded-lg hover:bg-green-700"
            >
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-auto lg:overflow-hidden">
        ...
        {/* Tool sidebar - vertical on desktop, horizontal on mobile (bottom) */}
        <aside className="hidden lg:flex w-14 bg-white border-r flex-col items-center py-3 gap-1 shrink-0">
          {([
          ...
          {(['red_pen', 'blue_pen', 'pencil'] as CorrectionStyle[]).map((s) => (
            ...
            >
              <div className="w-full h-full rounded-full" style={{ backgroundColor: STYLE_COLORS[s], opacity: s === 'pencil' ? 0.5 : 0.8 }} />
            </button>
          ))}
        </aside>

        {/* Paper image */}
        <div className="lg:flex-1 lg:overflow-auto p-2 sm:p-4">
          <div
          ...
          >
            {imageUrl && (
              <img
                ...
                className="w-full select-none rounded-lg shadow-sm"
                draggable={false}
              />
            )}
            ...
            {imageLoaded && annotations.map((ann) => {
              ...
              return (
                ...
                >
                  {ann.type === 'checkmark' && (
                    <span className="font-handwriting text-lg sm:text-2xl font-bold" style={{ color }}>✓</span>
                  )}
                  {ann.type === 'xmark' && (
                    <span className="font-handwriting text-lg sm:text-2xl font-bold" style={{ color }}>✗</span>
                  )}
                  {ann.type === 'circle' && (
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full border-2" style={{ borderColor: color }} />
                  )}
                  {ann.type === 'underline' && (
                    <div className="h-0.5" style={{ backgroundColor: color, width: `${(ann.bounds?.width ?? 0.1) * (imageSize.width)}px` }} />
                  )}
                  {ann.type === 'text_note' && (
                    <span className="font-handwriting text-xs sm:text-sm whitespace-nowrap" style={{ color }}>{ann.text}</span>
                  )}
                  {ann.type === 'score_box' && (
                    <span className="font-handwriting text-base sm:text-xl font-bold px-1.5 py-0.5 border-2 rounded" style={{ color, borderColor: color }}>{ann.text}</span>
                  )}
                  {ann.ai_generated && ann.confidence < 0.7 && (
                    <div className="absolute -top-1.5 -right-1.5 w-2.5 h-2.5 bg-amber-400 rounded-full" title="Low confidence" />
                  )}
                </div>
              );
            })}
        ...
        {/* AI Results panel - below image on mobile, right side on desktop */}
        <div className="lg:w-80 bg-white lg:border-l lg:overflow-y-auto shrink-0 border-t lg:border-t-0">
          ...
          <div className="divide-y">
            {questions.map((q) => (
              <div key={q.number} className="p-3 sm:p-4 hover:bg-gray-50">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs text-gray-500">{q.score}/{q.max_score}</span>
                  </div>
                </div>
                {q.question_text && (
                ...
                {q.is_correct === false && q.correct_answer && (
                  <p className="text-xs text-green-700 mt-1">Correct: {q.correct_answer}</p>
                )}
                {q.correction_note && (
      ...
      <div className="lg:hidden bg-white border-t px-2 py-2 flex items-center justify-center gap-1 shrink-0">
        {([
          { tool: 'select' as Tool, icon: Eye },
          { tool: 'checkmark' as Tool, icon: Check },
          { tool: 'xmark' as Tool, icon: X },
          { tool: 'circle' as Tool, icon: Circle },
          { tool: 'underline' as Tool, icon: Minus },
          { tool: 'text_note' as Tool, icon: Type },
          { tool: 'score_box' as Tool, icon: Hash },
        ]).map(({ tool, icon: Icon }) => (
          <button
            key={tool}
            onClick={() => setActiveTool(tool)}
            className={cn(
              ...
              activeTool === tool ? 'bg-brand-100 text-brand-700' : 'text-gray-500'
            )}
          >
        ...
        {(['red_pen', 'blue_pen', 'pencil'] as CorrectionStyle[]).map((s) => (
          <button
            ...
            className={cn(
              ...
              correctionStyle === s ? 'border-gray-900' : 'border-transparent'
            )}
          >
            <div className="w-full h-full rounded-full" style={{ backgroundColor: STYLE_COLORS[s], opacity: s === 'pencil' ? 0.5 : 0.8 }} />
          </button>
